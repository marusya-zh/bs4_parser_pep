...
from utils import find_sibling, find_tag, get_response
...
def pep(session):
    ...
    for pep_ref in tqdm(pep_refs):
        key = find_sibling(pep_ref, 'prev').text[1:]
        ...
        for dt_tag in dt_tags:
            if 'Status' in dt_tag.text:
                status = find_sibling(dt_tag).text
                keys_statuses_urls.append((key, status, pep_url))
                break

    statuses = [key_status_url[1] for key_status_url in keys_statuses_urls]
    cnt = Counter(statuses)
    peps_per_status = list(cnt.items())
